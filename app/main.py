...
async def curate_content(request: CurationRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        content = await curator.get_curated_content(request.model_dump())
        insights = curator.highlight_sentiment_insights(content) if request.include_sentiment else {}
        return convert_to_json(content, insights)
